...
ITRF2008       = ['D11','E11',      "ITRF2008"] #Modelo Geodesico       
ITRF2000AS     = ['D23','E23',    "ITRF2000AS"] #Modelo Geodesico
ITRF2000DA     = ['D29','E29',    "ITRF2000DA"] #Modelo Geodesico
APKIM2005_DGFI = ['D13','E13',"APKIM2005_DGFI"] #Modelo Geodesico
APKIM2005_IGN  = ['D15','E15', "APKIM2005_IGN"] #Modelo Geodesico
APKIM2000      = ['D27','E27',     "APKIM2000"] #Modelo Geodesico
CGPS2004       = ['D19','E19',      "CGPS2004"] #Modelo Geodesico
REVEL2000      = ['D21','E21',     "REVEL2000"] #Modelo Geodesico
GEODVEL2010    = ['D7','E7',     "GEODVEL2010"] #Modelo Geodesico
...
plantilla = load_wb('D:/TABLA VECTORIAL - ITRF2008_SA(NNR).xlsx')
...
Vector_sheet = vector_book.create_sheet(ITRF2008[2], 0)                                     # crea el objeto hoja de la clase hoja                                                                                   # Asigna un string al atributo titulo del objeto hoja
...
for file in os.listdir(path_mmodel):
    ...
    Vector_sheet.cell(row=counter, column=6).value = sheet_ranges[ITRF2000DA[0]].value    
    Vector_sheet.cell(row=counter, column=7).value = sheet_ranges[ITRF2000DA[1]].value  
    counter = counter + 1
    print(sheetname)
...
for file in os.listdir(path_vector):
    ...
    Vector_sheet.cell(row=counter, column=8).value = round(sheet_ranges.cell(row=9, column=3).value,2)
    Vector_sheet.cell(row=counter, column=9).value = round(sheet_ranges.cell(row=9, column=4).value,2)
    counter = counter + 1
    print(file)

##Guardado de la tabla vectorial
vector_book.save(path_out + ITRF2000DA[2] + '_' + relacion + '.xlsx')